...
export default function POSPage() {
  ...
  const importOrderToBill = async (order: any) => {
    ...
    const billItems: BillItem[] = (items || []).map((i: any) => ({
      ...
      name: i.item_name, price: i.item_price, quantity: i.quantity, note: i.notes || ''
    }))
    const bill: Bill = {
      id: order.id, label: 'Meja ' + order.table_number,
      items: billItems, customer: 'Guest',
      createdAt: new Date(order.created_at),
      status: 'open', payMethod: 'qris', discount: 0, fromOrderId: order.id
    }
    setBills(prev => prev.find(b => b.id === order.id) ? prev : [...prev, bill])
    setActiveBillId(bill.id)
    setActiveTab('pos')
  ...
  const confirmPayment = async () => {
    ...
    if (activeBill.fromOrderId) {
      await sb.from('orders').update({
        ...
        payment_method: payMethod, payment_confirmed_at: new Date().toISOString()
      }).eq('id', activeBill.fromOrderId)
    }
    setCompletedOrders(prev => [{ ...activeBill, status: 'paid', payMethod, total, paidAt: new Date(), cashReceived: parseInt(cashReceived) || 0 }, ...prev])
    setBills(prev => prev.filter(b => b.id !== activeBillId))
    setActiveBillId(null); setShowPayModal(false); setDiscount('')
    showToast('✅ Lunas!' + (payMethod === 'cash' ? ' Kembalian: ' + formatRp((parseInt(cashReceived) || 0) - total) : ''))
  }
  ...
  const S = { // inline styles helper
    btn: (active: boolean) => ({ padding: '7px 16px', borderRadius: 10, border: '1.5px solid', borderColor: active ? '#C8873A' : 'rgba(0,0,0,0.1)', background: active ? '#C8873A' : 'white', color: active ? 'white' : '#8B7355', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' } as React.CSSProperties),
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 60px)', overflow: 'hidden' }}>
      ...
      {/* TABS */}
      <div style={{ display: 'flex', gap: 6, paddingBottom: 14, flexShrink: 0 }}>
        {(['pos', 'bills', 'completed'] as const).map(t => (
          <button key={t} onClick={() => setActiveTab(t)} style={S.btn(activeTab === t)}>
        ...
        <div style={{ display: 'flex', gap: 14, flex: 1, overflow: 'hidden', minHeight: 0 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 8, alignContent: 'start' }}>
              {filteredMenu.map(m => {
                ...
                return (
                  <div key={m.id} onClick={() => addItem(m)} style={{ background: 'white', border: `1.5px solid ${qty > 0 ? '#C8873A' : 'rgba(0,0,0,0.08)'}`, borderRadius: 12, padding: '12px 10px', cursor: 'pointer', position: 'relative', userSelect: 'none' }}>
          ...
          <div style={{ width: 300, display: 'flex', flexDirection: 'column', flexShrink: 0, background: 'white', borderRadius: 16, border: '1px solid rgba(0,0,0,0.08)', overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(0,0,0,0.08)', flexShrink: 0 }}>
              <div style={{ fontSize: 11, color: '#8B7355', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.08em', marginBottom: 8 }}>Bill Aktif</div>
              {bills.filter(b => b.status === 'open').length === 0
                ? <div style={{ fontSize: 13, color: '#8B7355', textAlign: 'center', padding: '10px 0' }}>Belum ada bill. Klik <strong>+ New Bill</strong></div>
                : <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
                    {bills.filter(b => b.status === 'open').map(b => (
                      <button key={b.id} onClick={() => setActiveBillId(b.id)} style={{ padding: '5px 12px', borderRadius: 8, border: '1.5px solid', borderColor: activeBillId === b.id ? '#2C1810' : 'rgba(0,0,0,0.1)', background: activeBillId === b.id ? '#2C1810' : 'white', color: activeBillId === b.id ? 'white' : '#1A0F0A', fontWeight: 600, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                        {b.label} {b.items.length > 0 && <span style={{ opacity: 0.7 }}>({b.items.length})</span>}
                      </button>
                    ))}
                  </div>
              }
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '10px 16px' }}>
              {!activeBill
                ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#8B7355', fontSize: 13, gap: 8 }}><div style={{ fontSize: 32 }}>🧾</div><div>Pilih atau buat bill baru</div></div>
                : activeBill.items.length === 0
                  ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#8B7355', fontSize: 13, gap: 8 }}><div style={{ fontSize: 32 }}>+</div><div>Klik menu untuk tambah item</div></div>
                  : activeBill.items.map(item => (
                      <div key={item.menuItemId} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <button onClick={() => changeQty(item.menuItemId, -1)} style={{ width: 24, height: 24, borderRadius: 6, border: '1px solid rgba(0,0,0,0.12)', background: 'white', cursor: 'pointer', fontSize: 14 }}>−</button>
                          <span style={{ fontSize: 13, fontWeight: 700, minWidth: 18, textAlign: 'center' as const }}>{item.quantity}</span>
                          <button onClick={() => changeQty(item.menuItemId, 1)} style={{ width: 24, height: 24, borderRadius: 6, border: '1px solid rgba(0,0,0,0.12)', background: 'white', cursor: 'pointer', fontSize: 14 }}>+</button>
                        </div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#C8873A', minWidth: 65, textAlign: 'right' as const }}>{formatRp(item.price * item.quantity)}</div>
                      </div>
                    ))
              }
            </div>
            ...
            {activeBill && (
              <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(0,0,0,0.08)', flexShrink: 0 }}>
                <input value={discount} onChange={e => setDiscount(e.target.value)} placeholder="Diskon (Rp atau %)"
                  style={{ width: '100%', padding: '7px 12px', borderRadius: 8, border: '1.5px solid rgba(0,0,0,0.1)', fontSize: 13, fontFamily: 'inherit', outline: 'none', marginBottom: 10 }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#8B7355', marginBottom: 4 }}><span>Subtotal</span><span>{formatRp(getSubtotal(activeBill))}</span></div>
                {getDiscountAmount(activeBill) > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#E24B4A', marginBottom: 4 }}><span>Diskon</span><span>-{formatRp(getDiscountAmount(activeBill))}</span></div>}
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 17, fontWeight: 800, marginBottom: 12 }}><span>Total</span><span style={{ color: '#C8873A' }}>{formatRp(getTotal(activeBill))}</span></div>
                ...
                <button onClick={openPayment} disabled={activeBill.items.length === 0} style={{ width: '100%', padding: 12, borderRadius: 12, background: '#C8873A', border: 'none', color: 'white', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit', opacity: activeBill.items.length === 0 ? 0.5 : 1 }}>Proses Pembayaran</button>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {pendingOrders.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, color: '#8B7355', marginBottom: 10 }}>Order Masuk dari Pelanggan</div>
              {pendingOrders.map(o => (
                <div key={o.id} style={{ background: '#FFF8F0', border: '1.5px solid #C8873A', borderRadius: 12, padding: 16, display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: '#C8873A' }}>Meja {o.table_number}</div>
                    <div style={{ fontSize: 12, color: '#8B7355', marginTop: 2 }}>{o.order_number} · {formatTime(o.created_at)} · <span style={{ fontWeight: 600 }}>{o.status}</span></div>
                  </div>
                  <div style={{ textAlign: 'right' as const }}>
                    <div style={{ fontWeight: 700, fontSize: 15 }}>{formatRp(o.total_amount)}</div>
                    <button onClick={() => importOrderToBill(o)} style={{ marginTop: 6, padding: '6px 14px', borderRadius: 8, background: '#C8873A', border: 'none', color: 'white', fontWeight: 600, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Buka di POS →</button>
                  </div>
                </div>
              ))}
            </div>
          )}
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, color: '#8B7355', marginBottom: 10 }}>Open Bills</div>
          {bills.filter(b => b.status === 'open').length === 0
            ? <div style={{ background: 'white', borderRadius: 14, padding: 40, textAlign: 'center', color: '#8B7355' }}>Belum ada bill aktif</div>
            : bills.filter(b => b.status === 'open').map(b => (
                <div key={b.id} style={{ background: 'white', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 16, display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
                  ...
                  <div style={{ textAlign: 'right' as const }}>
                    ...
                    <button onClick={() => { setActiveBillId(b.id); setActiveTab('pos') }} style={{ marginTop: 6, padding: '6px 14px', borderRadius: 8, background: '#2C1810', border: 'none', color: 'white', fontWeight: 600, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Buka →</button>
                  </div>
                </div>
              ))
          }
      ...
      {activeTab === 'completed' && (
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, color: '#8B7355', marginBottom: 10 }}>Transaksi Selesai Hari Ini</div>
          {completedOrders.length === 0
            ? <div style={{ background: 'white', borderRadius: 14, padding: 40, textAlign: 'center', color: '#8B7355' }}>Belum ada transaksi</div>
            : <>
                {completedOrders.map((o: any, i: number) => (
                  <div key={i} style={{ background: 'white', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 16, display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 14 }}>{o.label}</div>
                      <div style={{ fontSize: 12, color: '#8B7355', marginTop: 2 }}>{o.customer || 'Guest'} · {o.payMethod?.toUpperCase()} · {o.items?.length} item</div>
                    </div>
                    <div style={{ textAlign: 'right' as const }}>
                ...
                <div style={{ background: '#FDF4E9', borderRadius: 12, padding: 16, textAlign: 'center' as const }}>
                  <div style={{ fontSize: 12, color: '#8B7355' }}>Total Hari Ini</div>
        ...
        <div onClick={e => e.target === e.currentTarget && setShowNewBillModal(false)}
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 380 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: '#8B7355', textTransform: 'uppercase' as const, display: 'block', marginBottom: 6 }}>Nomor Meja *</label>
              <input value={newBillTable} onChange={e => setNewBillTable(e.target.value)} placeholder="cth: 5 atau Take Away" onKeyDown={e => e.key === 'Enter' && createBill()}
        ...
        <div onClick={e => e.target === e.currentTarget && setShowPayModal(false)}
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 380 }}>
            ...
            {payMethod === 'cash' && (
              <>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ fontSize: 11, fontWeight: 700, color: '#8B7355', display: 'block', marginBottom: 6 }}>Uang yang Diterima</label>
                  <input value={cashReceived} onChange={e => setCashReceived(e.target.value)} type="number" placeholder="0"
                    style={{ width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid rgba(0,0,0,0.1)', fontSize: 18, fontFamily: 'inherit', outline: 'none', textAlign: 'center' as const }} />
                </div>
            ...
            {payMethod === 'qris' && <div style={{ textAlign: 'center' as const, padding: '20px 0', fontSize: 13, color: '#8B7355' }}><div style={{ fontSize: 48, marginBottom: 8 }}>📱</div>Arahkan kamera ke QRIS. Konfirmasi setelah pelanggan bayar.</div>}
            {payMethod === 'transfer' && <div style={{ textAlign: 'center' as const, padding: '20px 0', fontSize: 13, color: '#8B7355' }}><div style={{ fontSize: 48, marginBottom: 8 }}>🏦</div>Cek mutasi rekening. Konfirmasi setelah transfer masuk.</div>}
